...
type KB = {
  initiatives: Record<
    string,
    { id: string; name: string; stages: Record<string, { name: string }> }
  >;
  buying_jobs: Record<string, { name: string }>;
};
...
export default function Page() {
  ...
  useEffect(() => {
    async function loadKb() {
      try {
        ...
        // default initiative to the first key
  ...
  // Buying job options: use the canonical stage IDs we mapped in kb_dummy_v1.json
        ...
        {result && (
          <div style={{ display: "grid", gap: 14 }}>
            ...
            <div style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 10, padding: 12 }}>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {result.recommended_assets?.map((a: any) => (
                  <div key={a.rank} style={{ border: "1px solid #e5e5e5", borderRadius: 10, padding: 10, background: "white" }}>
            ...
            <div style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 10, padding: 12 }}>
              ...
              <div style={{ marginTop: 10 }}>
                ...
                <ol style={{ margin: "6px 0 0 18px" }}>
                  {result.top_asset_build_spec?.outline?.map((s: any, i: number) => (
                    <li key={i} style={{ marginBottom: 8 }}>
                      <div style={{ fontWeight: 800 }}>{s.section_title}</div>
                      <div style={{ color: "#444" }}>{s.section_goal}</div>
                      <ul style={{ margin: "6px 0 0 18px" }}>
                        {s.required_elements?.map((el: string, j: number) => (
                          <li key={j}>{el}</li>
                        ))}
                      </ul>
                    </li>
            ...
            <details style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 10, padding: 12 }}>
              <summary style={{ cursor: "pointer", fontWeight: 700 }}>Proof, Objections, Traceability (expand)</summary>
              <div style={{ marginTop: 10 }}>
                <div style={{ fontWeight: 800, marginBottom: 6 }}>Proof requirements</div>
                <pre style={{ whiteSpace: "pre-wrap", fontSize: 12, background: "white", padding: 10, borderRadius: 8, border: "1px solid #eee" }}>
{JSON.stringify(result.proof_requirements, null, 2)}
                </pre>

                <div style={{ fontWeight: 800, margin: "10px 0 6px" }}>Objection handling</div>
                <pre style={{ whiteSpace: "pre-wrap", fontSize: 12, background: "white", padding: 10, borderRadius: 8, border: "1px solid #eee" }}>
{JSON.stringify(result.objection_handling, null, 2)}
                </pre>

                <div style={{ fontWeight: 800, margin: "10px 0 6px" }}>Traceability</div>
                <pre style={{ whiteSpace: "pre-wrap", fontSize: 12, background: "white", padding: 10, borderRadius: 8, border: "1px solid #eee" }}>
{JSON.stringify(result.traceability, null, 2)}
                </pre>
              </div>
            </details>
          </div>
